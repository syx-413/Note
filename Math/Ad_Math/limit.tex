\section{极限(重要极限、夹逼准则、拉格朗日中值定理、洛必达法则、泰勒公式)}

极限运算分为函数极限和数列极限，数列极限和函数极限可以相互转化，这里主要以函数极限作为示例。

\paragraph{\boxed{\text{基本计算方式}}}

课本上极限计算可以使用的主要计算方式：

\paragraph{\boxed{\text{基础四则运算}}}

只有式子的极限各自存在才能使用四则运算，使用的频率较少。

\paragraph{\boxed{\text{重要极限}}}
...
\paragraph{\boxed{\text{导数定义:}}}

极限转换以及连续性的时候会用到，但是使用的频率也较小。

\paragraph{\boxed{\text{等价无穷小替换}}}
...
\subsection{常用化简技巧(对数法则、指数法则、三角函数关系式、和差形式)}

\paragraph{\boxed{\text{对数法则}}}
...
解：$\lim\limits_{x\to+\infty}(x+\sqrt{1+x^2})^{\frac{1}{x}}=e^{\lim\limits_{x\to+\infty}\frac{\ln(x+\sqrt{1+x^2})}{x}} \rightarrow
\left(\ln(x+\sqrt{1+x^2})'=\dfrac{1}{\sqrt{1+x^2}}\right)$\medskip
...
\paragraph{\boxed{\text{倒代换}}}

\subsubsection{\boxed{\text{含有分式}}}
...
\subsubsection{\boxed{\text{\texorpdfstring{$\infty-\infty$}\ 型}}}

\subsubsection{\boxed{\text{\texorpdfstring{$\infty\cdot\infty$}\ 型}}}

\paragraph{\boxed{\text{拆项}}}

拆项需要根据式子形式进行，所以很难找到普遍规律。

\subsubsection{\boxed{\text{积拆项}}}
...
\paragraph{\boxed{\text{极限转换}}}

\subsubsection{\boxed{\text{整体换元}}}
...
\subsubsection{\boxed{\text{无穷小}}}

求无穷小阶数时，注意低阶吸收高阶，即面对多项式的无穷小，其阶数为幂次最低的那个，逼近0速度最慢的那个的阶数。

\paragraph{\boxed{\text{等价无穷小}}} \leavevmode \medskip
...
\paragraph{\boxed{\text{某阶无穷小}}} \leavevmode \medskip
...
\paragraph{\boxed{\text{函数连续性}}}

函数的连续性代表：极限值=函数值。所以函数的连续性需要靠极限完成。

\subsubsection{\boxed{\text{极限判连续性}}}
...
\subsubsection{\boxed{\text{连续性求极限}}}
...
\paragraph{\boxed{\text{迭代式数列}}}

\subsubsection{\boxed{\text{简单递推表达式}}}
...
\subsubsection{\boxed{\text{单调有界准则}}}
...
\paragraph{\boxed{\text{通项公式}}} \leavevmode \medskip
...
\paragraph{\boxed{\text{复杂递推公式}}} \leavevmode \medskip
